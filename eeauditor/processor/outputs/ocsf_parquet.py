...
logger = logging.getLogger("OCSF_Parquet_Output")
...
@ElectricEyeOutput
class OcsfParquetOutput(object):
    __provider__ = "ocsf_parquet"
    ...
    def write_findings(self, findings: list, output_file: str, **kwargs):
        ...
        logger.info(
            "Converting %s findings into OCSF v1.7.0 events for Parquet output",
            len(findings)
        )
