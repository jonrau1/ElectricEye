...
def servicenow_sspm_active_user_mfa_check(cache: dict, awsAccountId: str, awsRegion: str, awsPartition: str):
    """
    [Servicenow.Users.1] Active users should have multi-factor authentication enabled
    """
    ...
    for user in get_servicenow_users(cache):
        ...
        if user["enable_multifactor_authn"] == "false":
            finding = {
                ...
                "Severity": {"Label": "HIGH"},
                "Confidence": 99,
                "Title": "[Servicenow.Users.1] Active users should have multi-factor authentication enabled",
...
def servicenow_sspm_active_user_failed_login_audits_check(cache: dict, awsAccountId: str, awsRegion: str, awsPartition: str):
    """
    [Servicenow.Users.2] Active users with more than {SNOW_FAILED_LOGIN_BREACHING_RATE} failed login attempts should be audited
    """
    iso8601Time = datetime.datetime.now(datetime.timezone.utc).isoformat()
    ...
    for user in get_servicenow_users(cache):
        ...
        if failedAttempts >= int(SNOW_FAILED_LOGIN_BREACHING_RATE):
            finding = {
                ...
                "Confidence": 99,
                "Title": f"[Servicenow.Users.2] Active users with more than {SNOW_FAILED_LOGIN_BREACHING_RATE} failed login attempts should be audited",
...
def servicenow_sspm_active_user_lockout_audit_check(cache: dict, awsAccountId: str, awsRegion: str, awsPartition: str):
    """
    [Servicenow.Users.3] Active users that are locked out should be audited
    """
    iso8601Time = datetime.datetime.now(datetime.timezone.utc).isoformat()
    ...
    for user in get_servicenow_users(cache):
        ...
        if user["locked_out"] == "true":
            finding = {
                ...
                "Severity": {"Label": "MEDIUM"},
                "Confidence": 99,
                "Title": "[Servicenow.Users.3] Active users that are locked out should be audited",
