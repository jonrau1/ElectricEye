...
def crawler_s3_encryption_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
...
@registry.register_check("glue")
def crawler_cloudwatch_encryption_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    crawler = list_crawlers(cache=cache)
    myCrawlers = crawler['CrawlerNames']
    for crawlers in myCrawlers:
        crawlerName = str(crawlers)
...
@registry.register_check("glue")
def crawler_job_bookmark_encryption_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    crawler = list_crawlers(cache=cache)
    myCrawlers = crawler['CrawlerNames']
    for crawlers in myCrawlers:
        crawlerName = str(crawlers)
...
@registry.register_check("glue")
def glue_data_catalog_encryption_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
...
@registry.register_check("glue")
def glue_data_catalog_password_encryption_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    catalogArn = 'arn:aws:glue:' + awsRegion + ':' + awsAccountId + ':catalog'
    # ISO Time
    iso8601Time = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()
    try:
...
@registry.register_check("glue")
def glue_data_catalog_resource_policy_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    catalogArn = 'arn:aws:glue:' + awsRegion + ':' + awsAccountId + ':catalog'
    # ISO Time
    iso8601Time = datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()
    try:
