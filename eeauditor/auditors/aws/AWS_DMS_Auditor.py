...
def dms_replication_instance_public_access_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
...
@registry.register_check("dms")
def dms_replication_instance_multi_az_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    # loop through dms replication instances
    response = dms.describe_replication_instances()
    for repinstances in response['ReplicationInstances']:
        dmsInstanceId = str(repinstances['ReplicationInstanceIdentifier'])
...
@registry.register_check("dms")
def dms_replication_instance_minor_version_update_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    # loop through dms replication instances
    response = dms.describe_replication_instances()
    for repinstances in response['ReplicationInstances']:
        dmsInstanceId = str(repinstances['ReplicationInstanceIdentifier'])
