...
def describe_images(cache, awsAccountId):
    ...
    if response:
        return response
    cache["describe_images"] = ec2.describe_images(Filters=[ { 'Name': 'owner-id','Values': [ awsAccountId ] } ],DryRun=False)
    return cache["describe_images"]


@registry.register_check("ami")
def public_ami_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
...
@registry.register_check("ami")
def encrypted_ami_check(cache: dict, awsAccountId: str, awsRegion: str) -> dict:
    amis = describe_images(cache=cache, awsAccountId=awsAccountId)
    myAmis = amis['Images']
    for ami in myAmis:
        imageId = str(ami['ImageId'])
