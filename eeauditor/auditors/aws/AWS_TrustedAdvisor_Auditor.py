...
def trusted_advisor_failing_root_mfa_check(cache: dict, session, awsAccountId: str, awsRegion: str, awsPartition: str) -> dict:
    ...
    try:
        ...
        if error.response['Error']['Code'] == 'SubscriptionRequiredException':
            ...
            print(error)
    # this is a passing check due to no matches
    except KeyError:
        finding = {
            "SchemaVersion": "2018-10-08",
            "Id": f"{checkArn}/trusted-advisor-failing-root-mfa-check",
...
@registry.register_check("support")
def trusted_advisor_failing_elb_listener_security_check(cache: dict, session, awsAccountId: str, awsRegion: str, awsPartition: str) -> dict:
