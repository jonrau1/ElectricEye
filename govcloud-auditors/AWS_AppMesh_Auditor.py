...
try:
    response = appmesh.list_meshes()
    myMesh = response["meshes"]
except Exception as e:
    print(e)


class AppmeshMeshEgressCheck(Auditor):
    def execute(self):
...
class AppmeshVirtNodeBackendDefaultTLSpolicyCheck(Auditor):
    def execute(self):
        for meshes in myMesh:
            meshName = str(meshes["meshName"])
            try:
                ...
                for nodes in response["virtualNodes"]:
                    ...
                    try:
                        ...
                        backendDefaultsCheck = str(
                            response["virtualNode"]["spec"]["backendDefaults"][
                                "clientPolicy"
                            ]
                        )
                        if backendDefaultsCheck == "{}":
                            ...
                            finding = {
                                "SchemaVersion": "2018-10-08",
                                "Id": nodeArn
                                + "/appmesh-virtual-node-default-tls-policy-check",
                                "ProductArn": "arn:aws-us-gov:securityhub:"
                                + awsRegion
                                + ":"
                                + awsAccountId
...
class AppmeshVirtNodeListenerStrictTLScheck(Auditor):
    def execute(self):
        for meshes in myMesh:
            meshName = str(meshes["meshName"])
            try:
...
class AppmeshLoggingCheck(Auditor):
    def execute(self):
        for meshes in myMesh:
            meshName = str(meshes["meshName"])
            try:
                ...
                for nodes in response["virtualNodes"]:
                    ...
                    try:
                        ...
                        finding = {
                            "SchemaVersion": "2018-10-08",
                            "Id": nodeArn
                            + "/appmesh-virtual-node-access-logging-check",
                            "ProductArn": "arn:aws-us-gov:securityhub:"
                            + awsRegion
                            + ":"
                            + awsAccountId
