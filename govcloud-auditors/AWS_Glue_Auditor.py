...
class CrawlerS3EncryptionCheck(Auditor):
    def execute(self):
        for crawlers in myCrawlers:
            ...
            try:
                ...
                crawlerSecConfig = str(
                    response["Crawler"]["CrawlerSecurityConfiguration"]
                )
                try:
...
class CrawlerCloudwatchEncryptionCheck(Auditor):
    def execute(self):
        for crawlers in myCrawlers:
            crawlerName = str(crawlers)
            crawlerArn = (
                ...
                try:
                    ...
                    if cwEncryptionCheck == "DISABLED":
                        finding = {
                            "SchemaVersion": "2018-10-08",
                            "Id": crawlerArn
                            + "/glue-crawler-cloudwatch-encryption-check",
                            "ProductArn": "arn:aws-us-gov:securityhub:"
                            + awsRegion
                            + ":"
                            + awsAccountId
...
class CrawlerJobBookmarkEncryptionCheck(Auditor):
    def execute(self):
        for crawlers in myCrawlers:
            crawlerName = str(crawlers)
            crawlerArn = (
                ...
                try:
                    ...
                    if jobBookmarkEncryptionCheck == "DISABLED":
                        finding = {
                            "SchemaVersion": "2018-10-08",
                            "Id": crawlerArn
                            + "/glue-crawler-job-bookmark-encryption-check",
                            "ProductArn": "arn:aws-us-gov:securityhub:"
                            + awsRegion
                            + ":"
                            + awsAccountId
...
class GlueDataCatalogEncryptionCheck(Auditor):
    def execute(self):
        catalogArn = (
            "arn:aws-us-gov:glue:" + awsRegion + ":" + awsAccountId + ":catalog"
        )
        # ISO Time
        iso8601Time = (
...
class GlueDataCatalogPasswordEncryptionCheck(Auditor):
    def execute(self):
        catalogArn = (
            "arn:aws-us-gov:glue:" + awsRegion + ":" + awsAccountId + ":catalog"
        )
...
class GlueDataCatalogResourcePolicyCheck(Auditor):
    def execute(self):
        catalogArn = (
            "arn:aws-us-gov:glue:" + awsRegion + ":" + awsAccountId + ":catalog"
        )
