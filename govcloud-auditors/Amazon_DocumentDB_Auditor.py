...
response = documentdb.describe_db_instances(
    Filters=[{"Name": "engine", "Values": ["docdb"]}], MaxRecords=100
)
myDocDbs = response["DBInstances"]


class DocdbPublic_Instance_Check(Auditor):
    def execute(self):
...
class DocdbInstanceEncryptionCheck(Auditor):
    def execute(self):
        for docdb in myDocDbs:
            docdbId = str(docdb["DBInstanceIdentifier"])
            docdbArn = str(docdb["DBInstanceArn"])
...
class DocdbInstanceAuditLoggingCheck(Auditor):
    def execute(self):
        for docdb in myDocDbs:
            docdbId = str(docdb["DBInstanceIdentifier"])
            docdbArn = str(docdb["DBInstanceArn"])
...
class DocdbClusterMultiazCheck(Auditor):
    def execute(self):
...
class DocdbClusterDeletionProtectionCheck(Auditor):
    def execute(self):
        # find document db instances
        response = documentdb.describe_db_clusters(MaxRecords=100)
        myDocDbClusters = response["DBClusters"]
...
class DocumentdbParameterGroupTlsEnforcementCheck(Auditor):
    def execute(self):
        response = documentdb.describe_db_cluster_parameter_groups()
        dbClusterParameters = response["DBClusterParameterGroups"]
        for parametergroup in dbClusterParameters:
...
class DocumentdbClusterSnapshotEncryptionCheck(Auditor):
    def execute(self):
...
class DocumentdbClusterSnapshotPublicShareCheck(Auditor):
    def execute(self):
        response = documentdb.describe_db_clusters(
            Filters=[{"Name": "engine", "Values": ["docdb"]}]
        )
