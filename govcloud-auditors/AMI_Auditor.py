...
response = ec2.describe_images(
    Filters=[{"Name": "owner-id", "Values": [awsAccount]}], DryRun=False
)
myAmis = response["Images"]


class PublicAMICheck(Auditor):
    def execute(self):
        for ami in myAmis:
            imageId = str(ami["ImageId"])
            amiArn = "arn:aws-us-gov:ec2:" + awsRegion + "::image/" + imageId
...
class EncryptedAMICheck(Auditor):
    def execute(self):
        for ami in myAmis:
            imageId = str(ami["ImageId"])
            amiArn = "arn:aws-us-gov:ec2:" + awsRegion + "::image/" + imageId
