...
class EbsVolumeAttachmentCheck(Auditor):
    def execute(self):
...
class EbsVolumeDeleteOnTerminationCheck(Auditor):
    def execute(self):
        for volumes in myEbsVolumes:
            ebsVolumeId = str(volumes["VolumeId"])
            ebsVolumeArn = (
...
class EbsVolumeEncryptionCheck(Auditor):
    def execute(self):
        for volumes in myEbsVolumes:
            ebsVolumeId = str(volumes["VolumeId"])
            ebsVolumeArn = (
...
class EbsSnapshotEncryptionCheck(Auditor):
    def execute(self):
        for snapshots in myEbsSnapshots:
            snapshotId = str(snapshots["SnapshotId"])
            snapshotArn = "arn:aws-us-gov:ec2:" + awsRegion + "::snapshot/" + snapshotId
...
class EbsSnapshotPublicCheck(Auditor):
    def execute(self):
        for snapshots in myEbsSnapshots:
            snapshotId = str(snapshots["SnapshotId"])
            snapshotArn = "arn:aws-us-gov:ec2:" + awsRegion + "::snapshot/" + snapshotId
...
class EbsAccountEncryptionByDefaultCheck(Auditor):
    def execute(self):
