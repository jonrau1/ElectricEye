...
class VolumeDeleteOnTerminationCheck():
    def except(self):
        for volumes in myEbsVolumes:
            ebsVolumeId = str(volumes['VolumeId'])
            ebsVolumeArn = 'arn:aws:ec2:' + awsRegion + ':' + awsAccountId + '/' + ebsVolumeId
...
class AccountEncryptionByDefaultCheck():
    def except(self):
