...
def lambda_handler(event, context):
    ...
    for findings in securityHubEvent:
        ...
        for resources in findings['Resources']:
            ...
            # create resource ID
            eipAllocId = resourceId.replace('arn:aws:ec2:' + awsRegion + ':' + findingOwner + ':eip-allocation/', '')
            if findingOwner != masterAccountId:
                ...
                try:
                    # release EIP
                    response = ec2.release_address(AllocationId=eipAllocId,DryRun=False)
                    try:
                        response = securityhub.update_findings(
                            Filters={'Id': [{'Value': findingId,'Comparison': 'EQUALS'}]},
                            Note={'Text': 'Unused Elastic IP has been released and the finding was archived.','UpdatedBy': lambdaFunctionName},
