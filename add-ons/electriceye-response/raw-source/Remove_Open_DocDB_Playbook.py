    ...
    for findings in securityHubEvent:
        ...
        for resources in findings['Resources']:
            ...
            if findingOwner != masterAccountId:
                ...
                try:
                    # remove 0.0.0.0 access to 27017
                    response = security_group.revoke_ingress(IpPermissions=[{'IpProtocol': 'tcp','FromPort': 27017,'ToPort': 27017,'IpRanges': [{'CidrIp': '0.0.0.0/0'}]}],DryRun=False)
                    try:
                        response = securityhub.update_findings(
                            Filters={'Id': [{'Value': findingId,'Comparison': 'EQUALS'}]},
                            Note={'Text': 'Removed open access to DocumentDB (TCP 27017) from Security Group and archived the finding. Review the Security Group for any other unauthorized port changes.','UpdatedBy': lambdaFunctionName},
