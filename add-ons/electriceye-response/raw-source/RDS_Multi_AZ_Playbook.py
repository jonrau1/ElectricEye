    ...
    for findings in securityHubEvent:
        ...
        for resources in findings['Resources']:
            ...
            if findingOwner != masterAccountId:
                ...
                try:
                    # enable RDS multi-az
                    response = rds.modify_db_instance(DBInstanceIdentifier=rdsInstance,MultiAZ=True,ApplyImmediately=True)
                    print(response)
                    try:
                        response = securityhub.update_findings(
                            Filters={'Id': [{'Value': findingId,'Comparison': 'EQUALS'}]},
                            Note={'Text': 'Mutli-AZ has been configured for the RDS instance and the finding was archived. Review the RDS Instance for any other unauthorized configurations.','UpdatedBy': lambdaFunctionName},
