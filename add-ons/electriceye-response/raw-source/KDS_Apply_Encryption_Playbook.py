...
def lambda_handler(event, context):
    ...
    for findings in securityHubEvent:
        ...
        for resources in findings['Resources']:
            ...
            # create resource ID
            kdsName = resourceId.replace('arn:aws:kinesis:' + awsRegion + ':' + findingOwner + ':stream/', '')
            if findingOwner != masterAccountId:
                ...
                kinesis = boto3.client('kinesis',aws_access_key_id=xAcctAccessKey,aws_secret_access_key=xAcctSecretKey,aws_session_token=xAcctSeshToken)
                try:
                    # put default KDS encryption
                    response = kinesis.start_stream_encryption(StreamName=kdsName,EncryptionType='KMS',KeyId='alias/aws/kinesis')
                    print(response)
                    try:
                        response = securityhub.update_findings(
                            Filters={'Id': [{'Value': findingId,'Comparison': 'EQUALS'}]},
                            Note={'Text': 'AWS managed KMS encryption has been applied to the Kinesis stream and the finding was archived. Review the stream for any other configurations that should be applied and consider creating a KMS-CMK specific for the stream if it is business-critical.','UpdatedBy': lambdaFunctionName},
